...
\textbf{Stream ciphers} are an example of \textbf{symmetric encryption}. Each digit in a \textbf{plaintext stream} is encrypted with a digit of the \textbf{keystream}. Examples of stream ciphers include \textbf{Caesar} and \textbf{Viginere} ciphers as well as \textbf{one time pad}. \np
...
The Vigenere ciper uses the representation used by most stream ciphers, that `A' = 0, `B' = 1 ... 'Z' = 25, allowing for modulo 26 arithmetic. A \textbf{running key} is used, in which the complete key is a string repeated to the length of the plaintext. For example the full key for initial key `run' used to encode string `hello' is `runru.' Encoding then takes place using:
\[
	C = P + K\ mod\ 26
\]
...
\end{enumerate}
\textbf{Vulnerabilities} can exist where websites supply forged or expired certificates, or if a browser passes responsibility for checking an invalid certificate to the user who will often just click OK.
